...
function s3_head() {
    local S3_PATH=$1
    shift
    # TODO: ca-bundle instead of insecure?
    curl --aws-sigv4 "aws:amz:$S3_ENDPOINT:s3" --user "$AWS_ACCESS_KEY_ID:$AWS_SECRET_ACCESS_KEY" --insecure --fail --silent \
        "$@" \
        --head "$S3_URL/$S3_PATH"
}
...
function s3_cp() {
    ...
    curl --aws-sigv4 "aws:amz:$S3_ENDPOINT:s3" --user "$AWS_ACCESS_KEY_ID:$AWS_SECRET_ACCESS_KEY" --insecure --fail --silent \
        ...
        --header "x-amz-content-sha256: $(sha256sum < "$TEMPNAME" | cut -f1 -d' ')" \
        "$@" \
