...
static constexpr char help_string[] =
    ...
    "\n"
    "   putheader_mp_threshold (default=multipart_threshold value is used)\n"
    "      - threshold in MB for using multipart header upload(put header).\n"
    "        If this option is not specified, the value of multipart_threshold\n"
    "        is used. Specify this option to change the threshold for multipart\n"
    "        upload and header upload. This value must be at least 5MB.\n"
    "\n"
    ...
    "\n"
    "   sigv2 (default is signature version 4 only)\n"
    "      - sets signing AWS requests by using only signature version 2\n"
    "\n"
    "   sigv4 (default is signature version 4 only)\n"
    ...
    "      The APIKey may be a plain IBM API key or a Service ID API key.\n"
    "      When combined with ibm_trusted_profile_id, the AWSSecretKey must\n"
    "      be the API key of a Service ID that is linked to the Trusted Profile.\n"
    "\n"
    "   ibm_iam_endpoint (default is https://iam.cloud.ibm.com)\n"
    "      - sets the URL to use for IBM IAM authentication.\n"
    "\n"
    "   ibm_trusted_profile_id (default=\"\" which means disabled)\n"
    "      - Specifies the IBM Trusted Profile ID to scope the IBM IAM token\n"
    "      to the permissions of the given Trusted Profile.\n"
    "      Setting this option automatically enables IBM IAM authentication\n"
    "      (ibm_iam_auth), so ibm_iam_auth does not need to be set separately.\n"
    "      The passwd file must contain the Service ID API key in the form:\n"
    "        :<service-id-api-key>\n"
    "      Example: ibm_trusted_profile_id=Profile-xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx\n"
    "\n"
    ...
    "\n"
    "   hard_remove (remove objects which are unlinked while open)\n"
    "        By default, a file which is removed while another process has\n"
    "        it open is renamed to a hidden \".fuse_hidden*\" object and\n"
    "        removed when it is finally closed, which costs an extra server\n"
    "        side copy. If this option is specified, the object is removed\n"
    "        immediately and s3fs keeps serving the open file handles from\n"
    "        the local cache. Pending writes to such a file are discarded\n"
    "        at close, because the object no longer exists.\n"
    "\n"
