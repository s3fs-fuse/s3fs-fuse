...
static constexpr char help_string[] =
    ...
    "\n"
    "   fastreaddir (skip per-object HEAD in readdir/getattr)\n"
    "        Populate the stat cache directly from ListObjects results during\n"
    "        readdir, instead of issuing one HEAD per entry. Subsequent stat\n"
    "        calls hit the cache; on a cache miss a single HEAD is issued and\n"
    "        only its Content-Length and Last-Modified fields are used.\n"
    "        x-amz-meta-* headers (mode, uid, gid, mtime, atime, ctime, xattr)\n"
    "        and Content-Type-based directory detection are ignored, so file\n"
    "        permissions and ownership use the global uid/gid/umask defaults\n"
    "        and symlinks appear as regular files. Trades POSIX metadata\n"
    "        fidelity for substantially fewer S3 requests on large directories.\n"
    "\n"
