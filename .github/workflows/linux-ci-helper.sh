...
if [ "${CONTAINER_FULLNAME}" = "ubuntu:25.04" ]; then
    ...
    INSTALL_PACKAGES="autoconf autotools-dev openjdk-21-jre-headless fuse3 jq libfuse3-dev libcurl4-openssl-dev libxml2-dev locales-all mailcap libtool pkg-config libssl-dev attr curl procps python3-pip unzip"
    ...
    INSTALL_PACKAGES="bash clang-extra-tools curl g++ make automake autoconf libtool git curl-dev fuse3 fuse3-dev jq libxml2-dev openssl coreutils procps attr sed mailcap openjdk21 perl-test-harness-utils aws-cli"
